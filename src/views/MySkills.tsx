...
export function MySkills() {
  ...
  const [restoreVersionTag, setRestoreVersionTag] = useState<string | null>(
    null,
  )
  const [restoringVersionTag, setRestoringVersionTag] = useState<string | null>(
    null,
  )
  ...
  const viewedScenarioName =
    viewedScenario?.name || t('mySkills.currentScenarioFallback')
  ...
  useEffect(() => {
    ...
    api
      .getScenarioSkillOrder(viewedScenario.id)
      .then(setScenarioSkillOrder)
      .catch(() => {})
  ...
  const filtered = useMemo(() => {
    const result = skills.filter((skill) => {
      ...
      if (tagFilters.size > 0 && !skill.tags.some((t) => tagFilters.has(t)))
        return false
  ...
  const {
    ...
    getKey: (s) => s.id,
    isItemActive: (s) =>
      viewedScenario ? s.scenario_ids.includes(viewedScenario.id) : true,
  })
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      const enabledSkills = filtered.filter((s) =>
        s.scenario_ids.includes(viewedScenario.id),
      )
      ...
      try {
        await api.reorderScenarioSkills(
          viewedScenario.id,
      ...
      if (committed || status.ahead > 0) {
        ...
        toast.success(
          t('mySkills.gitSyncSuccessWithVersion', {
            tag: displaySnapshotLabel(snapshotTag),
          }),
        )
      } else {
  ...
  const handleRestoreVersion = async () => {
    ...
    try {
      ...
      toast.success(
        t('mySkills.gitVersionRestoreSuccess', {
      ...
      <span
        ...
        title={[
          gitStatus.has_changes || gitStatus.ahead > 0
            ? t('mySkills.gitInlineLocalChanges', {
                count: Math.max(gitStatus.ahead, gitStatus.has_changes ? 1 : 0),
              })
            : null,
          gitStatus.behind > 0
            ? t('mySkills.gitInlineRemoteUpdates', { count: gitStatus.behind })
            : null,
        ]
          .filter(Boolean)
          .join(' · ')}
      ...
      <div className="app-toolbar">
        ...
        <div className="app-segmented">
          {(() => {
            ...
            const snapshotWhen = formatSnapshotWhen(
              gitStatus?.current_snapshot_tag ?? null,
            )
            return (
              <>
                {inlineStatus ? (
                ...
                {gitStatus?.is_repo ? (
                  <button
                    ...
                    title={
                      snapshotWhen
                        ? t('mySkills.gitInlineLastSnapshot', {
                            when: snapshotWhen,
                          })
                        : undefined
                    }
      ...
      {gitVersionsOpen && gitStatus?.is_repo && (
        <div className="app-panel -mt-2 mb-2 p-3">
          <div className="mb-2 flex items-center justify-between">
            ...
            >
              <RefreshCw
                className={cn('h-3 w-3', gitVersionsLoading && 'animate-spin')}
              />
              {t('settings.refresh')}
            </button>
          </div>
                ...
                >
                  <div className="min-w-0 pr-3">
                    ...
                    <div className="text-[11px] text-faint">
                      {version.commit} ·{' '}
                      {formatGitDateTime(version.committed_at)}
                    </div>
                  </div>
                  <button
                ...
                ? (() => {
                    ...
                    if (viewMode === 'grid') {
                      return (
                        <div
                          className={cn(
                            'app-panel relative flex h-full cursor-grabbing flex-col shadow-xl opacity-40',
                    ...
                    return (
                      <div
                        className={cn(
                          'app-panel relative flex cursor-grabbing items-center gap-3.5 rounded-xl border-transparent px-3.5 py-3 shadow-xl opacity-40',
                          activeSkill.scenario_ids.includes(
                            viewedScenario?.id ?? '',
                          ) && 'border-l-2 border-l-accent',
                        )}
