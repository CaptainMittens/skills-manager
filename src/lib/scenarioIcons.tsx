...
const SCENARIO_KEYWORD_RULES: Array<{ key: string; keywords: string[] }> = [
  ...
  {
    key: 'book-open',
    keywords: ['学习', 'study', 'learn', 'course', 'research'],
  },
  {
    key: 'folder-git-2',
    keywords: ['开源', 'opensource', 'open source', 'github'],
  },
  ...
  {
    key: 'terminal',
    keywords: ['cli', 'terminal', 'shell', 'command', '命令行'],
  },
  { key: 'database', keywords: ['data', 'database', 'sql', '数据'] },
  {
    key: 'chart-bar',
    keywords: ['analytics', 'metric', 'report', 'dashboard', '分析'],
  },
  ...
  {
    key: 'calendar',
    keywords: ['plan', 'planning', 'schedule', 'calendar', '计划'],
  },
  ...
  {
    key: 'server',
    keywords: ['infra', 'infrastructure', 'server', 'ops', '运维'],
  },
...
export function inferScenarioIconKey(
  scenario?: Pick<Scenario, 'name' | 'description' | 'icon'> | null,
) {
  ...
  const haystack =
    `${scenario?.name || ''} ${scenario?.description || ''}`.toLowerCase()
...
export function getScenarioIconOption(
  ...
  const key =
    typeof scenario === 'string' ? scenario : inferScenarioIconKey(scenario)
